...
def get_lang_detector():
    ...
    if _LANG_DETECTOR_INSTANCE is None:
        ...
        # 设置模型目录 - 使用相对路径修复
        # ModuleFolders/Domain/FileReader/ReaderUtil.py -> 根目录 (回退3层)
        base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        model_dir = os.path.join(base_dir, "Resource", "Models", "mediapipe")
        model_path = os.path.join(model_dir, "language_detector.tflite")

        # 自动下载逻辑
        if not os.path.exists(model_path):
            _print(f"[[yellow]WARNING[/]] 模型文件不存在，尝试自动下载...")
            try:
                ...
                _print(f"[[cyan]INFO[/]] Downloading from {url}...")
                urllib.request.urlretrieve(url, model_path)
                _print(f"[[green]SUCCESS[/]] 模型已下载至: {model_path}")
            except Exception as e:
                _print(f"[[red]ERROR[/]] 自动下载失败: {e}")
                raise FileNotFoundError(f"无法找到或下载模型文件: {model_path}")
        ...
        try:
            ...
            _print(f"[[green]INFO[/]] MediaPipe 文本语言检测器已加载! ({load_time_ms:.2f} ms)")

        except Exception as e:
            _print(f"[[red]ERROR[/]] 加载 MediaPipe 语言检测器失败: {e}")
            # 重新抛出异常，以便调用者知道出了问题
            # 或者根据您的应用程序进行适当处理。
            raise
...
def close_lang_detector():
    ...
    if _LANG_DETECTOR_INSTANCE is not None:
        ...
        try:
            ...
            _print("[[green]INFO[/]] MediaPipe 文本语言检测器已释放!")
...
def detect_file_encoding(file_path: Union[str, pathlib.Path], min_confidence: float = 0.75) -> str:
    ...
    try:
        ...
        _print(f"[[red]ERROR[/]] 文件 {file_path} 编码检测过程出错: {str(e)}")
        return 'utf-8'
